...
## Order response_type
detection$response_type <- ordered(factor(detection$response_type), c("optimal", "suboptimal", "low-virulence"))
...
detect_summary_select <- detect_summary[c("iteration", "farms_detected", "first_detect", "last_detect", "duration")]

## Check data
head(detect_summary_select, 3)
```

### Summary Statistics
`generate_detect_statistics()` returns summary statistics for each modeling scenario. Results are grouped by `region`, `response_type`, and `preclinical`.
```{r}
...
duration_central_summary <- duration_central_summary[c("response_type", "preclinical", "mean", "q05", "q25", "q50", "q75", "q95")]
...
## Select and order columns
duration_eastern_summary <- duration_eastern_summary[c("response_type", "preclinical", "mean", "q05", "q25", "q50", "q75", "q95")]
...
range(no_LV_detect_central_summary$duration)
plot(density(no_LV_detect_central_summary$duration))
```
...
range(no_LV_detect_eastern_summary$duration)
plot(density(no_LV_detect_eastern_summary$duration))
```
...
## Central U.S.
```{r}
model_central_duration <- lm(duration ~ preclinical * response_type, 
...
## Eastern U.S.
```{r}
model_eastern_duration <- lm(duration ~ preclinical * response_type, 
