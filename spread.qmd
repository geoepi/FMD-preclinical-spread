...
## Plot data
ggplot(daily_Re, aes(infect_day, cred_50, group = preclinical, col = preclinical)) +
...
The analyses below include tables and figures for the Eastern U.S. scenarios only. Visuals for the Central U.S. scenarios are coming soon.
...
## Cumulative Spread
Calculate the cumulative spatial spread of simulated outbreaks. First, drop the low-virulence scenario to compare optimal vs. suboptimal detection scenarios.
```{r}
iteration_metrics_no_low <- iteration_metrics %>%
  filter(scenario_type != "Low-Virulence")
```

Plot the estimated Area Under the Curve (AUC), which represents total cumulative spread distance, for each scenario by iteration (points). Then run a linear regression model to find the trend across increasing preclinical infectious duration.
...
### Significance Test
Fit a linear model with an interaction term to quantify the influence of `preclinical` and `scenario_type` on cumulative outbreak spread (`auc_log`).
