...
### Summary Statistics
`generate_infect_statistics()` returns summary statistics for each modeling scenario. Results are grouped by `region`, `response_type`, and `preclinical`. The output is filtered by `summary` and `region` to compare `farms_infected` and `cattle_infected` between scenarios.
```{r}
...
`plot_infected_cattle()` generates a plot with the duration of incubation phase transmission on the x-axis and the median number of infected cattle (log10) on the y-axis.
