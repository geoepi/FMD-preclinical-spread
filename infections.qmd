...
infect_summary_select <- infect_summary[c("iteration", "farms_infected", "cattle_infected", "first_infect_day", "last_infect_day")]

## Check data
head(infect_summary_select, 3)
```

### Summary Statistics
The `generate_infect_statistics()` returns summary statistics grouped by region, preclinical infectious duration, and response type. The `infect_config_summary` can be filtered by `summary` to summarize `farms_infected` or `cattle_infected` across scenarios.
```{r}
...
farms_central_summary <- farms_central_summary[c("response_type", "preclinical", "mean", "q05", "q25", "q50", "q75", "q95")]
...
## Select and order columns
farms_eastern_summary <- farms_eastern_summary[c("response_type", "preclinical", "mean", "q05", "q25", "q50", "q75", "q95")]
...
## Select and order columns
cattle_central_summary <- cattle_central_summary[c("response_type", "preclinical", "mean", "q05", "q25", "q50", "q75", "q95")]
...
## Select and order columns
cattle_eastern_summary <- cattle_eastern_summary[c("response_type", "preclinical", "mean", "q05", "q25", "q50", "q75", "q95")]
...
## Significance Test
Significance testing performed on optimal and suboptimal response types for each region
...
## Central U.S.
```{r}
model_central_cattle <- lm(log(cattle_infected) ~ preclinical * response_type, 
...
## Eastern U.S.
```{r}
model_eastern_cattle <- lm(log(cattle_infected) ~ preclinical * response_type, 
...
The `plot_infected_cattle()` plots infected cattle by scenario, with the duration of incubation phase transmission on the x-axis and the median number of infected cattle on the y-axis.
